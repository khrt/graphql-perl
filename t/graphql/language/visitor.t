...
subtest 'visit_with_type_info' => sub {
    ...
    subtest 'maintains type info during visit' => sub {
        my @visited;
        my $type_info = GraphQL::Util::TypeInfo->new($test_schema);
        ...
        visit($ast, visit_with_typeinfo($type_info, {
            enter => sub {
                ...
                push @visited, [
                    'enter',
                    $node->{kind},
                    $node->{kind} eq 'Name' ? $node->{value}          : undef,
                    $parent_type            ? $parent_type->to_string : undef,
                    $type                   ? $type->to_string        : undef,
                    $input_type             ? $input_type->to_string  : undef
                ];
