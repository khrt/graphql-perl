...
subtest 'visit_with_type_info' => sub {
    subtest 'maintains type info during visit' => sub {
        ...
        my $type_info = GraphQL::TypeInfo->new($test_schema);
        ...
        visit($ast, visit_with_typeinfo($type_info, {
            enter => sub {
                ...
                push @visited, [
                    ...
                    $parent_type            ? $parent_type->to_string : NULL,
                    $type                   ? $type->to_string        : NULL,
                    $input_type             ? $input_type->to_string  : NULL
                ];
    ...
    subtest 'maintains type info during edit' => sub {
        ...
        my $edited_ast = visit($ast, visit_with_typeinfo($type_info, {
            enter => sub {
                ...
                if (   $node->{kind} eq 'Field'
                    && !$node->{selection_set}
                    && is_composite_type(get_named_type($type)))
                {
                    return {
                        kind => 'Field',
                        alias => $node->{alias},
        ...
        is_deeply print_doc($ast), print_doc(parse(
            '{ human(id: 4) { name, pets }, alien }'
        ));

        is_deeply print_doc($edited_ast), print_doc(parse(
